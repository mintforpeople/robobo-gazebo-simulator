...
void MovePanTilt::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  model->GetJoint("tilt_motor")->SetLowerLimit(0, 70*M_PI/180);
  model->GetJoint("tilt_motor")->SetUpperLimit(0, 70*M_PI/180);
  model->GetJoint("pan_motor")->SetPosition(0, M_PI);
  model->GetJoint("pan_motor")->SetLowerLimit(0, M_PI);
  model->GetJoint("pan_motor")->SetUpperLimit(0, M_PI);
                                    ...
                                    int8_t tiltSpeed, int16_t tiltBlockId)
{
  ...
  double currentPanAngle = model->GetJoint("pan_motor")->Position()*180/M_PI;
  double currentTiltAngle = model->GetJoint("tilt_motor")->Position()*180/M_PI;
  ...
  if (10 < panPos && panPos < 345 && panPos != round(currentPanAngle) && panBlockId != 0)
  {
    ...
    model->GetJoint("pan_motor")->SetLowerLimit(0, 0.191);
    model->GetJoint("pan_motor")->SetUpperLimit(0, 6.004);
    // TODO: Busy waiting could be prevented with events on Gazebo 8+
    if (currentPanAngle < panPos)
    {
      while (model->GetJoint("pan_motor")->Position(0)*180/M_PI < panPos)
      {
    ...
    model->GetJoint("pan_motor")->SetLowerLimit(0, model->GetJoint("pan_motor")->Position(0));
    model->GetJoint("pan_motor")->SetUpperLimit(0, model->GetJoint("pan_motor")->Position(0));
  ...
  if (4 < tiltPos && tiltPos < 111 && tiltPos != round(currentTiltAngle) && tiltBlockId != 0)
  {
    ...
    // Unlock joint position and set target velocity
    model->GetJoint("tilt_motor")->SetLowerLimit(0, 0.0873);
    model->GetJoint("tilt_motor")->SetUpperLimit(0, 1.9199);
    // TODO: This busy wait could be prevented from Gazebo 8+
    if (currentTiltAngle < tiltPos)
    {
      while (model->GetJoint("tilt_motor")->Position(0)*180/M_PI < tiltPos)
      {
    ...
    model->GetJoint("tilt_motor")->SetLowerLimit(0, model->GetJoint("tilt_motor")->Position(0));
    model->GetJoint("tilt_motor")->SetUpperLimit(0, model->GetJoint("tilt_motor")->Position(0));
