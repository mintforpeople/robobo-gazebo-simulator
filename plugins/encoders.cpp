...
void Encoders::Callback(const common::UpdateInfo &)
{
  ...
  {
    int newPanPos = int(round(model->GetJoint("pan_motor")->Position()*180/M_PI));
    if (panPos != newPanPos)
    {
      panPos = newPanPos;
      msgPan.data = panPos;
  ...
  {
    int newTiltPos = int(round(model->GetJoint("tilt_motor")->Position()*180/M_PI));
...
{
  rReset = round(model->GetJoint("right_motor")->Position()*180/M_PI);
  lReset = round(model->GetJoint("left_motor")->Position()*180/M_PI);
}
