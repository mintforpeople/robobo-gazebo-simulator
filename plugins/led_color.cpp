...
void LedColorRobobo::TopicCallback(const boost::shared_ptr<robobo_msgs::Led const> &req)
{
  ...
  req->id.data=="Front-L" || req->id.data=="Front-LL" || req->id.data=="Back-R" || req->id.data=="Back-L"){
    ...
    for (int i=1;i<8;i++){
      if (regex_search(this->visual->GetChild(i)->GetName(), ledChildId)){
        id_led=i;
      }
    }
  }
...
void LedColorRobobo::colorCallback(int id_led, float r, float g, float b){
  ...
  if (id_led!=10){
    if (r!=-1){
      gazebo::common::Color newColor= gazebo::common::Color(r,g,b,1.0);
      gazebo::common::Color onColor= gazebo::common::Color(0,0,0,1.0);
      ...
      gazebo::common::Color newColor= gazebo::common::Color(0.0,0.0,0.0,0.0);
